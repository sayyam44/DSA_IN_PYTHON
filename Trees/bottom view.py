...
# https://www.geeksforgeeks.org/bottom-view-binary-tree/#:~:text=Given%20a%20Binary%20Tree%2C%20we,node%20at%20its%20horizontal%20distance.
# sc=n,tc=n
...
#append all the values in form of node,hd into the queue and all its child nodes along with left's child's
#level -1,-2,-3 and also add its right child along with its level +1,+2,...
...
def bottomview(root):
    ...
    while q:
        #in the below steps we are poping the top value of que and storing it in the opposite way into dict m 
        temp=q[0]
        q.pop()
        hd=temp.hd
    ...
    for i in sorted(m.keys()):   # Traverse the map elements using the iterator in sorted way
        print(m[i], end = ' ')
